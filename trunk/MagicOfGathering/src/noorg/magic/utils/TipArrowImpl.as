package noorg.magic.utils
{
	...
	public class TipArrowImpl implements ITipArrow
	{
		...
		{
			if (arrowDirection == TipArrowUtil.LEFT) {
				g.lineTo(startX - this.tipHeight, this.distanceC);
			} else if (arrowDirection == TipArrowUtil.TOP) {
				g.lineTo(this.distanceC, startY - this.tipHeight);
			} else if (arrowDirection == TipArrowUtil.RIGHT) {
				g.lineTo(startX + this.tipHeight, this.distanceC);
			} else if (arrowDirection == TipArrowUtil.BOTTOM) {
				g.lineTo(this.distanceC, startY + this.tipHeight);
			}
		}
